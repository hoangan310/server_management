<section class="w-full">
    <x-page-heading>
        <x-slot:title>{{ __('users.title') }}</x-slot:title>
        <x-slot:subtitle>{{ __('users.title_description') }}</x-slot:subtitle>
        <x-slot:buttons>
            @can('create users')
            ...
            <x-table.row wire:key="user-{{ $user->id }}">
                ...
                <x-table.cell class="gap-2 flex justify-end">
                    ...
                    <flux:modal.trigger name="delete-user-modal">
                        <flux:button size="sm" variant="danger" wire:click="confirmDelete({{ $user->id }})">
                            {{ __('global.delete') }}
                        </flux:button>
                    </flux:modal.trigger>
                    @endcan
